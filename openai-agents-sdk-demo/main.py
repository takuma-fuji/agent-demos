...
question_deepener_agent = Agent(
    ...
    instructions=add_handoff_info(
        ...
        "最後に研究計画のまとめを箇条書きで示し、小学生にもわかりやすい手順と準備物を説明してください。そのうえで『exit』または『quit』と入力すれば終了できると伝えてください。"
    ),
...
question_decider_agent = Agent(
    ...
    instructions=add_handoff_info(
        "あなたは自由研究のテーマを提案する先生です。渡された興味リストを読み取り、\n"
        "ネット検索をして、興味に関連する面白そうなテーマ案を列挙してください。\n"
        "生徒との会話を繰り返して、テーマ案のうち最も興味を持ってもらえそうなものを選び、\n"
...
interest_finder_agent = Agent(
    ...
    instructions=add_handoff_info(
        "あなたは小学生への聞き取りが得意です。質問と生徒の回答を繰り返しながら、生徒の興味や好きなことを最大3個のキーワードにまとめてください。\n"
        "興味がまとまったら handoff ツール **`transfer_to_問い決定エージェント`** を呼び出してください。"
        "生徒の興味をJSON形式で **`transfer_to_問い決定エージェント`** に渡してください（例: {\"interests\": [\"昆虫\", \"宇宙\", \"水の実験\"]}）。\n\n"
...
self_intro_agent = Agent(
    ...
    instructions=add_handoff_info(
        "あなたは夏休みの自由研究サポートボットです。最初に丁寧な自己紹介を行い、これから一緒に研究テーマを見つける流れを簡潔に説明してください。\n\n"
...
def main() -> None:
    ...
    print("ボット: こんにちは！私は夏休みの自由研究を一緒に考えるお手伝いをします。まずは自己紹介してください。")
